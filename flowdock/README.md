...
현재 **61개 테스트 전부 통과 (네트워크 불필요)**, `tsc --noEmit` 클린.
...
.flowdock/               # 런타임 상태 (레지스트리·실행·웹훅시크릿; gitignore)
```
...
플랜·과금·레이트리밋 풀 + 팀 협업·권한. 로그 보존기간이 가장 자연스러운 페이월. 자세한 내용은 [`ARCHITECTURE.md`](./ARCHITECTURE.md).
